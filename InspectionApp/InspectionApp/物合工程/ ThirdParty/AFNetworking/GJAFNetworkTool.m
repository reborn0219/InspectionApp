...
#import "GJAFNetworkTool.h"

@implementation GJAFNetworkTool
...
+ (void)netWorkStatus
{
    ...
    [[GJAFNetworkReachabilityManager sharedManager] startMonitoring];
    
    // 检测网络连接的单例,网络变化时的回调方法
    [[GJAFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
...
{
    GJAFHTTPRequestOperationManager *manager = [GJAFHTTPRequestOperationManager manager];
    
    NSDictionary *dict = @{@"format": @"json"};
    // 网络访问是异步的,回调是主线程的,因此程序员不用管在主线程更新UI的事情
    [manager GET:url parameters:dict success:^(GJAFHTTPRequestOperation *operation, id responseObject) {
        if (success) {
...
+ (void)XMLDataWithUrl:(NSString *)urlStr success:(void (^)(id xml))success fail:(void (^)())fail
{
    ...
    // 网络访问是异步的,回调是主线程的,因此程序员不用管在主线程更新UI的事情
    [manager GET:urlStr parameters:dict success:^(GJAFHTTPRequestOperation *operation, id responseObject) {
        if (success) {
            success(responseObject);
        }
...
+ (void)postJSONWithUrl:(NSString *)urlStr parameters:(id)parameters success:(void (^)(id responseObject))success fail:(void (^)())fail
{
    ...
    [manager POST:urlStr parameters:parameters success:^(GJAFHTTPRequestOperation *operation, id responseObject) {
...
+ (void)sessionDownloadWithUrl:(NSString *)urlStr success:(void (^)(NSURL *fileURL))success fail:(void (^)())fail
{
    ...
    GJAFURLSessionManager *manager = [[GJAFURLSessionManager alloc] initWithSessionConfiguration:config];

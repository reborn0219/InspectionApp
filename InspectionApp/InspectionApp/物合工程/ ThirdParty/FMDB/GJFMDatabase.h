...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `GJFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<GJFMResultSet>` - Represents the results of executing a query on an `GJFMDatabase`.
 - `<GJFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<GJFMDatabasePool>` - A pool of `GJFMDatabase` objects.
 ...
 @warning Do not instantiate a single `GJFMDatabase` object and use it across multiple threads. Instead, use `<GJFMDatabaseQueue>`.

 */

@interface GJFMDatabase : NSObject  {
...
/** Create a `GJFMDatabase` object.
 
 An `GJFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `GJFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `GJFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    GJFMDatabase *db = [GJFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    GJFMDatabase *db     = [GJFMDatabase databaseWithPath:dbPath];
 ...
 @return `GJFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `GJFMDatabase` object.
...
/** Execute select statement
 ...
 Executing queries returns an `<GJFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[GJFMResultSet next]>`) from one record to the other.
 ...
 @return A `<GJFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see GJFMResultSet
 @see [`GJFMResultSet next`](<[GJFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (GJFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (GJFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (GJFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (GJFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (GJFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(GJFMDatabase *adb) {
        ...
        int rowCount = 0;
        GJFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [GJFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using GJFMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `FMStatement`, but rather with `<GJFMDatabase>` and `<GJFMResultSet>` only.

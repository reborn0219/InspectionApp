...
@interface GJFMDatabasePool()

- (void)pushDatabaseBackInPool:(GJFMDatabase*)db;
- (GJFMDatabase*)db;

@end


@implementation GJFMDatabasePool
...
- (void)pushDatabaseBackInPool:(GJFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The GJFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (GJFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [GJFMDatabase databaseWithPath:_path];

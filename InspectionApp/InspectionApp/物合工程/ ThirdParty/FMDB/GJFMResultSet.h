...
/** Represents the results of executing a query on an `<GJFMDatabase>`.
...
@interface GJFMResultSet : NSObject {
    GJFMDatabase          *_parentDB;
...
/** Create result set from `<FMStatement>`
 ...
 @param aDB A `<GJFMDatabase>` to be used
 
 @return A `GJFMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(FMStatement *)statement usingParentDatabase:(GJFMDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(GJFMDatabase *)newDb;

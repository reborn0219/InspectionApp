...
@interface GJFZPBusinessTabbarTypeView(){
...
@implementation GJFZPBusinessTabbarTypeView
...
-(void)initBtnOnView{
    ...
    for ( int i=0; i<ViewCount; i++) {
        ...
        button.frame=CGRectMake(((float)self.frame.size.width/(float)ViewCount*i), 0, (float)self.frame.size.width/(float)ViewCount, self.frame.size.height);
        ...
        if (i == 1) {
            self.redButton = [[UIButton alloc]initWithFrame:CGRectMake(((float)self.frame.size.width/(float)ViewCount)*2 - 13, 0, 10, 10)];
            [self.redButton setBackgroundColor:[UIColor redColor]];
            self.redButton.layer.cornerRadius = 5.0;

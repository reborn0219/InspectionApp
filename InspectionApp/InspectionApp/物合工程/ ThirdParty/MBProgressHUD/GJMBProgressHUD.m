...
@implementation GJMBProgressHUD {
	BOOL useAnimation;
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	GJMBProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	GJMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [GJMBProgressHUD allHUDsForView:view];
	for (GJMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (GJMBProgressHUD *)subview;
		}
	}
	return nil;
}

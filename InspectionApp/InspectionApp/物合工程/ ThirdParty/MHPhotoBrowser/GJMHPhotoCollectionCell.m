...
#import "GJMHPhotoCollectionCell.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import "GJMHPhotoBrowserController.h"
#import "GJSVProgressHUD.h"

@interface GJMHPhotoCollectionCell()<UIGestureRecognizerDelegate>
@property (weak, nonatomic) UIGestureRecognizer *recognizer;
@end

@implementation GJMHPhotoCollectionCell
...
{
    GJMHPhotoBrowserController *con = (GJMHPhotoBrowserController*)self.viewController;
    if ([con isKindOfClass:[GJMHPhotoBrowserController class]]) {
...
{
    if (error == nil) {
        [GJSVProgressHUD showSuccessWithStatus:@"保存成功"];
    }
}
...
- (void)createView
{
    ...
    self.photoView = [[GJMHTapImgView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
        ...
        if ([[[photo.photoURL scheme] lowercaseString] isEqualToString:@"assets-library"]) {
            ...
            [self.photoView sd_setImageWithURL:photo.photoURL completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                ...
                if (img) {
                    ...
                    [GJSVProgressHUD showErrorWithStatus:@"图片加载失败"];
                }
            }];
        }
    }

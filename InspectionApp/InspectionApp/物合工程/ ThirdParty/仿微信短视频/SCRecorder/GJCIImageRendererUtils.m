...
#import "GJCIImageRendererUtils.h"

@implementation GJCIImageRendererUtils
...
+ (CGRect)processRect:(CGRect)rect withImageSize:(CGSize)imageSize contentScale:(CGFloat)contentScale contentMode:(UIViewContentMode)mode {
    rect = [GJCIImageRendererUtils rect:rect byApplyingContentScale:contentScale];
...
+ (void)putUIImage:(UIImage *)image toRenderer:(id<CIImageRenderer>)renderer {
    if (image == nil) {
        ...
        renderer.preferredCIImageTransform = [GJCIImageRendererUtils preferredCIImageTransformFromUIImage:image];

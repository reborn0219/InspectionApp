...
@implementation GJSCAssetExportSession

-(instancetype)init {
    self = [super init];
    
    if (self) {
        _audioQueue = dispatch_queue_create("me.corsin.GJSCAssetExportSession.AudioQueue", nil);
        _videoQueue = dispatch_queue_create("me.corsin.GJSCAssetExportSession.VideoQueue", nil);
...
- (GJSCIOPixelBuffers *)createIOPixelBuffers:(CMSampleBufferRef)sampleBuffer {
    ...
    if (_outputBufferDiffersFromInput) {
        ...
        GJSCIOPixelBuffers *pixelBuffers = [GJSCIOPixelBuffers IOPixelBuffersWithInputPixelBuffer:inputPixelBuffer outputPixelBuffer:outputPixelBuffer time:time];
        CVPixelBufferRelease(outputPixelBuffer);
        
        return pixelBuffers;
    } else {
        return [GJSCIOPixelBuffers IOPixelBuffersWithInputPixelBuffer:inputPixelBuffer outputPixelBuffer:inputPixelBuffer time:time];
    }
}

- (GJSCIOPixelBuffers *)renderIOPixelBuffersWithCI:(GJSCIOPixelBuffers *)pixelBuffers {
    GJSCIOPixelBuffers *outputPixelBuffers = pixelBuffers;
    ...
    if (_context != nil) {
        ...
        outputPixelBuffers = [GJSCIOPixelBuffers IOPixelBuffersWithInputPixelBuffer:pixelBuffers.outputPixelBuffer outputPixelBuffer:pixelBuffers.outputPixelBuffer time:pixelBuffers.time];
    }
...
- (void)beginReadWriteOnVideo {
    if (_videoInput != nil) {
        GJSCProcessingQueue *videoProcessingQueue = nil;
        GJSCProcessingQueue *filterRenderingQueue = nil;
        GJSCProcessingQueue *videoReadingQueue = [GJSCProcessingQueue new];
        ...
        [videoReadingQueue startProcessingWithBlock:^id{
            ...
            GJSCSampleBufferHolder *holder = nil;
            
            if (sampleBuffer != nil) {
                holder = [GJSCSampleBufferHolder sampleBufferHolderWithSampleBuffer:sampleBuffer];
        ...
        if (_videoPixelAdaptor != nil) {
            filterRenderingQueue = [GJSCProcessingQueue new];
            filterRenderingQueue.maxQueueSize = 2;
            [filterRenderingQueue startProcessingWithBlock:^id{
                GJSCIOPixelBuffers *pixelBuffers = nil;
                GJSCSampleBufferHolder *bufferHolder = [videoReadingQueue dequeue];
            ...
            videoProcessingQueue = [GJSCProcessingQueue new];
            videoProcessingQueue.maxQueueSize = 2;
            [videoProcessingQueue startProcessingWithBlock:^id{
                GJSCIOPixelBuffers *videoBuffers = [filterRenderingQueue dequeue];
        ...
        [_videoInput requestMediaDataWhenReadyOnQueue:_videoQueue usingBlock:^{
            ...
            while (strongSelf.videoInput.isReadyForMoreMediaData && shouldReadNextBuffer && !strongSelf.cancelled) {
                GJSCIOPixelBuffers *videoBuffer = nil;
                GJSCSampleBufferHolder *bufferHolder = nil;
...
- (BOOL)_setupContextIfNeeded {
    if (_videoInput != nil && _filter != nil) {
        ...
        if (contextType == SCContextTypeAuto) {
            contextType = [GJSCContext suggestedContextType];
        }
        ...
        _context = [GJSCContext contextWithType:_contextType options:options];
...
+ (NSError*)createError:(NSString*)errorDescription {
    return [NSError errorWithDomain:@"GJSCAssetExportSession" code:200 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
}
...
- (GJSCFilter *)_generateRenderingFilterForVideoSize:(CGSize)videoSize {
    GJSCFilter *watermarkFilter = [self _buildWatermarkFilterForVideoSize:videoSize];
    GJSCFilter *renderingFilter = nil;
    GJSCFilter *customFilter = self.videoConfiguration.filter;

    if (customFilter != nil) {
        if (watermarkFilter != nil) {
            renderingFilter = [GJSCFilter emptyFilter];
...
- (GJSCFilter *)_buildWatermarkFilterForVideoSize:(CGSize)videoSize {
    ...
    if (watermarkImage != nil) {
        ...
        return [GJSCFilter filterWithCIImage:watermarkCIImage];
    }
...
- (void)exportAsynchronouslyWithCompletionHandler:(void (^)())completionHandler {
    ...
    _writer.metadata = [GJSCRecorderTools assetWriterMetadata];

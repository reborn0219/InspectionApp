...
- (id)copyWithZone:(NSZone *)zone {
    GJSCFilter *filter = [GJSCFilter emptyFilter];
    ...
    if (value == nil) {
        ...
        if ([key isEqualToString:@"inputCubeData"]) {
            if ([value isKindOfClass:[NSData class]]) {
                ...
                if ([GJSCFilter data:data hasMagic:MagicPNG]) {
                    image = CGImageCreateWithPNGDataProvider(source, nil, NO, kCGRenderingIntentDefault);
                } else if ([GJSCFilter data:data hasMagic:MagicJPG]) {
                ...
                value = [GJSCFilter colorCubeDataWithCGImage:image dimension:dimension];
...
- (void)resetToDefaults {
    ...
    for (GJSCFilter *subFilter in _subFilters) {
...
- (void)addSubFilter:(GJSCFilter *)subFilter {
    [_subFilters addObject:subFilter];
}

- (void)removeSubFilter:(GJSCFilter *)subFilter {
    [_subFilters removeObject:subFilter];
}

- (void)insertSubFilter:(GJSCFilter *)subFilter atIndex:(NSInteger)index {
...
- (GJSCFilterAnimation *)addAnimationForParameterKey:(NSString *)key startValue:(id)startValue endValue:(id)endValue startTime:(CFTimeInterval)startTime duration:(CFTimeInterval)duration {
    GJSCFilterAnimation *animation = [GJSCFilterAnimation filterAnimationForParameterKey:key startValue:startValue endValue:endValue startTime:startTime duration:duration];
...
- (void)addAnimation:(GJSCFilterAnimation *)animation {
    [_animations addObject:animation];
}

- (void)removeAnimation:(GJSCFilterAnimation *)animation {
...
- (CIImage *)imageByProcessingImage:(CIImage *)image atTime:(CFTimeInterval)time {
    ...
    for (GJSCFilter *filter in _subFilters) {
        image = [filter imageByProcessingImage:image atTime:time];
    }
    
    for (GJSCFilterAnimation *animation in _animations) {
...
+ (GJSCFilter *)emptyFilter {
    return [GJSCFilter filterWithCIFilter:nil];
}

+ (GJSCFilter *)filterWithAffineTransform:(CGAffineTransform)affineTransform {
    ...
    return [GJSCFilter filterWithCIFilter:filter];
}

+ (GJSCFilter *)filterWithCIFilter:(CIFilter *)filterDescription {
    return [[GJSCFilter alloc] initWithCIFilter:filterDescription];
}

+ (GJSCFilter *)filterWithCIFilterName:(NSString *)name {
    CIFilter *coreImageFilter = [CIFilter filterWithName:name];
    [coreImageFilter setDefaults];
    
    return coreImageFilter != nil ? [GJSCFilter filterWithCIFilter:coreImageFilter] : nil;
}

+ (GJSCFilter *)filterWithData:(NSData *)data error:(NSError *__autoreleasing *)error {
    id obj = nil;
    @try {
    ...
    if (![obj isKindOfClass:[GJSCFilter class]]) {
        obj = nil;
        if (error != nil) {
...
+ (GJSCFilter *)filterWithData:(NSData *)data {
    return [GJSCFilter filterWithData:data error:nil];
}

+ (GJSCFilter *)filterWithContentsOfURL:(NSURL *)url {
    NSData *data = [NSData dataWithContentsOfURL:url];
    
    if (data != nil) {
        return [GJSCFilter filterWithData:data];
    }
    
    return nil;
}

+ (GJSCFilter *)filterWithFilters:(NSArray *)filters {
    GJSCFilter *filter = [GJSCFilter emptyFilter];
    
    for (GJSCFilter *subFilter in filters) {
...
+ (GJSCFilter *)filterWithCIImage:(CIImage *)image {
    GJSCFilter *filter = [GJSCFilter emptyFilter];

...
@interface GJSCImageView()<GLKViewDelegate>

#else
@import MetalKit;

@interface GJSCImageView()<GLKViewDelegate, MTKViewDelegate>
...
@property (nonatomic, strong) GJSCSampleBufferHolder *sampleBufferHolder;

@end

@implementation GJSCImageView
...
- (void)_imageViewCommonInit {
    ...
    _sampleBufferHolder = [GJSCSampleBufferHolder new];
}
    ...
    if (_context == nil) {
        ...
        switch (contextType) {
            ...
            case SCContextTypeCPU:
                [NSException raise:@"UnsupportedContextType" format:@"GJSCImageView does not support CPU context type."];
                break;
            default:
                break;
        }

        self.context = [GJSCContext contextWithType:contextType options:options];
    }
...
- (void)setContext:(GJSCContext * _Nullable)context {
    [self unloadContext];
...
#endif
            default:
                [NSException raise:@"InvalidContext" format:@"Unsupported context type: %d. GJSCImageView only supports CoreGraphics, EAGL and Metal", (int)context.type];
                break;
        }
    }
...
- (void)setImageByUIImage:(UIImage *)image {
    if (image == nil) {
        ...
        self.preferredCIImageTransform = [GJSCImageView preferredCIImageTransformFromUIImage:image];

 ...
 */
- (void)player:(GJSCPlayer *__nonnull)player didPlay:(CMTime)currentTime loopsCount:(NSInteger)loopsCount;

/**
 Called when the item has been changed on the GJSCPlayer
 */
- (void)player:(GJSCPlayer *__nonnull)player didChangeItem:(AVPlayerItem *__nullable)item;

/**
 Called when the item has reached end
 */
- (void)player:(GJSCPlayer *__nonnull)player didReachEndForItem:(AVPlayerItem *__nonnull)item;

/**
 Called when the item is ready to play
 */
- (void)player:(GJSCPlayer *__nonnull)player itemReadyToPlay:(AVPlayerItem *__nonnull)item;

/**
 Called when the player has setup the renderer so it can receive the image in the
 proper orientation.
 */
- (void)player:(GJSCPlayer *__nonnull)player didSetupSCImageView:(GJSCImageView *__nonnull)GJSCImageView;
 ...
 */
@property (strong, nonatomic) GJSCImageView *__nullable GJSCImageView;

/**
 Convenient method to return a new instance of a GJSCPlayer
 */
+ (GJSCPlayer *__nonnull)player;

/**
 Ask the GJSCPlayer to send didPlay messages during the playback
 endSendingPlayMessages must be called, otherwise the GJSCPlayer will never
 be deallocated
 */
- (void)beginSendingPlayMessages;

/**
 Ask the GJSCPlayer to stop sending didPlay messages during the playback
 */

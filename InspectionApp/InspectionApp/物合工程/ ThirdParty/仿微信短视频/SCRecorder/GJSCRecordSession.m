...
@implementation GJSCRecordSession
...
- (id)initWithDictionaryRepresentation:(NSDictionary *)dictionaryRepresentation {
    ...
    if (self) {
        ...
        for (NSObject *recordSegment in recordSegments) {
            ...
            NSURL *url = [GJSCRecordSessionSegment segmentURLForFilename:filename andDirectory:_segmentsDirectory];
            
            if ([[NSFileManager defaultManager] fileExistsAtPath:url.path]) {
                [_segments addObject:[GJSCRecordSessionSegment segmentWithURL:url info:info]];
            } else {
        ...
        for (NSDictionary *segmentDictRepresentation in segments) {
            GJSCRecordSessionSegment *segment = [[GJSCRecordSessionSegment alloc] initWithDictionaryRepresentation:segmentDictRepresentation directory:_segmentsDirectory];
        ...
        if (shouldRecomputeDuration) {
            ...
            if (CMTIME_IS_INVALID(_segmentsDuration)) {
                ...
                NSLog(@"The imported GJSCRecordSession is probably not useable.");
            }
        }
...
- (id)init {
    ...
    if (self) {
        ...
        _identifier = [NSString stringWithFormat:@"%@-", [GJSCRecordSession newIdentifier:12]];
        _audioQueue = dispatch_queue_create("me.corsin.GJSCRecorder.Audio", nil);
    }
...
+ (id)recordSession {
    return [[GJSCRecordSession alloc] init];
}

+ (id)recordSession:(NSDictionary *)dictionaryRepresentation {
    return [[GJSCRecordSession alloc] initWithDictionaryRepresentation:dictionaryRepresentation];
}

+ (NSError*)createError:(NSString*)errorDescription {
    return [NSError errorWithDomain:@"GJSCRecordSession" code:200 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
}

- (void)dispatchSyncOnSessionQueue:(void(^)())block {
    GJSCRecorder *recorder = self.recorder;
    
    if (recorder == nil || [GJSCRecorder isSessionQueue]) {
        block();
    } else {
...
- (void)removeSegment:(GJSCRecordSessionSegment *)segment {
...
- (void)removeSegmentAtIndex:(NSInteger)segmentIndex deleteFile:(BOOL)deleteFile {
    [self dispatchSyncOnSessionQueue:^{
        GJSCRecordSessionSegment *segment = [_segments objectAtIndex:segmentIndex];
...
//            NSLog(@"Removed duration of %fs", CMTimeGetSeconds(segmentDuration));
            ...
            for (GJSCRecordSessionSegment *segment in _segments) {
...
- (void)removeAllSegments:(BOOL)removeFiles {
    [self dispatchSyncOnSessionQueue:^{
        while (_segments.count > 0) {
            if (removeFiles) {
                GJSCRecordSessionSegment *segment = [_segments objectAtIndex:0];
                [segment deleteFile];
            }
...
- (NSURL *)nextFileURL:(NSError **)error {
    ...
    if (extension != nil) {
        ...
        NSURL *file = [GJSCRecordSessionSegment segmentURLForFilename:filename andDirectory:self.segmentsDirectory];
        ...
        if (error != nil) {
            *error = [GJSCRecordSession createError:[NSString stringWithFormat:@"Unable to find an extension"]];
        }
...
- (AVAssetWriter *)createWriter:(NSError **)error {
    ...
    if (fileType != nil) {
        ...
        if (file != nil) {
            ...
            writer.metadata = [GJSCRecorderTools assetWriterMetadata];
        }
    } else {
        theError = [GJSCRecordSession createError:@"No fileType has been set in the GJSCRecordSession"];
    }
    ...
    if (theError == nil) {
        ...
        if (_videoInput != nil) {
            if ([writer canAddInput:_videoInput]) {
                ...
                theError = [GJSCRecordSession createError:@"Cannot add videoInput to the assetWriter with the currently applied settings"];
            }
        }
        ...
        if (_audioInput != nil) {
            if ([writer canAddInput:_audioInput]) {
                ...
                theError = [GJSCRecordSession createError:@"Cannot add audioInput to the assetWriter with the currently applied settings"];
            }
        }
...
- (void)initializeVideo:(NSDictionary *)videoSettings formatDescription:(CMFormatDescriptionRef)formatDescription error:(NSError *__autoreleasing *)error {
    ...
    @try {
        ...
        theError = [GJSCRecordSession createError:exception.reason];
    }
...
- (void)addSegment:(GJSCRecordSessionSegment *)segment {
    [self dispatchSyncOnSessionQueue:^{
        [_segments addObject:segment];
...
- (void)insertSegment:(GJSCRecordSessionSegment *)segment atIndex:(NSInteger)segmentIndex {
    [self dispatchSyncOnSessionQueue:^{
...
- (void)beginSegment:(NSError**)error {
    [self dispatchSyncOnSessionQueue:^{
        if (_assetWriter == nil) {
            ...
            if (error != nil) {
                *error = [GJSCRecordSession createError:@"A record segment has already began."];
            }
        }
    }];
}
...
- (void)appendRecordSegmentUrl:(NSURL *)url info:(NSDictionary *)info error:(NSError *)error completionHandler:(void (^)(GJSCRecordSessionSegment *, NSError *))completionHandler {
    [self dispatchSyncOnSessionQueue:^{
        GJSCRecordSessionSegment *segment = nil;
        
        if (error == nil) {
            segment = [GJSCRecordSessionSegment segmentWithURL:url info:info];
...
- (BOOL)endSegmentWithInfo:(NSDictionary *)info completionHandler:(void(^)(GJSCRecordSessionSegment *segment, NSError* error))completionHandler {
    ...
    [self dispatchSyncOnSessionQueue:^{
        dispatch_sync(_audioQueue, ^{
            if (_recordSegmentReady) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completionHandler != nil) {
                        completionHandler(nil, [GJSCRecordSession createError:@"The current record segment is not ready for this operation"]);
                    }
                });
            }
        });
...
- (void)beginRecordSegmentUsingMovieFileOutput:(AVCaptureMovieFileOutput *)movieFileOutput error:(NSError *__autoreleasing *)error delegate:(id<AVCaptureFileOutputRecordingDelegate>)delegate {
    ...
    if (url != nil) {
        ...
        _movieFileOutput.metadata = [GJSCRecorderTools assetWriterMetadata];
...
- (AVAssetExportSession *)mergeSegmentsUsingPreset:(NSString *)exportSessionPreset completionHandler:(void(^)(NSURL *outputUrl, NSError *error))completionHandler {
    ...
    [self dispatchSyncOnSessionQueue:^{
        ...
        if (fileType == nil) {
            error = [GJSCRecordSession createError:@"No output fileType was set"];
            return;
        }
        
        NSString *fileExtension = [self _suggestedFileExtension];
        if (fileExtension == nil) {
            error = [GJSCRecordSession createError:@"Unable to figure out a file extension"];
            return;
        }
        
        NSString *filename = [NSString stringWithFormat:@"%@SCVideo-Merged.%@", _identifier, fileExtension];
        outputUrl = [GJSCRecordSessionSegment segmentURLForFilename:filename andDirectory:_segmentsDirectory];
        [self removeFile:outputUrl];

        if (_segments.count == 0) {
            error = [GJSCRecordSession createError:@"The session does not contains any record segment"];
        } else {
...
- (void)cancelSession:(void (^)())completionHandler {
    [self dispatchSyncOnSessionQueue:^{
        if (_assetWriter == nil) {
            ...
            [self endSegmentWithInfo:nil completionHandler:^(GJSCRecordSessionSegment *segment, NSError *error) {
                [self removeAllSegments];
                if (completionHandler != nil) {
                    completionHandler();
                }
...
- (void)appendSegmentsToComposition:(AVMutableComposition *)composition audioMix:(AVMutableAudioMix *)audioMix {
    [self dispatchSyncOnSessionQueue:^{
        ...
        for (GJSCRecordSessionSegment *recordSegment in _segments) {
...
- (AVPlayerItem *)playerItemRepresentingSegments {
    ...
    [self dispatchSyncOnSessionQueue:^{
        if (_segments.count == 1) {
            GJSCRecordSessionSegment *segment = _segments.firstObject;
...
- (NSDictionary *)dictionaryRepresentation {
    ...
    for (GJSCRecordSessionSegment *recordSegment in self.segments) {
...
- (NSURL *)outputUrl {
    ...
    return [GJSCRecordSessionSegment segmentURLForFilename:filename andDirectory:_segmentsDirectory];
}
...
- (void)setSegmentsDirectory:(NSString *)segmentsDirectory {
    ...
    [self dispatchSyncOnSessionQueue:^{
        ...
        for (GJSCRecordSessionSegment *recordSegment in self.segments) {
            NSURL *newUrl = [GJSCRecordSessionSegment segmentURLForFilename:recordSegment.url.lastPathComponent andDirectory:_segmentsDirectory];

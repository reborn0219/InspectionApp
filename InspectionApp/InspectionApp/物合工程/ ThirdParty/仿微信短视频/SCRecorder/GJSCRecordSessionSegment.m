...
@interface GJSCRecordSessionSegment() {
    AVAsset *_asset;
    UIImage *_thumbnail;
    UIImage *_lastImage;
}
...
@implementation GJSCRecordSessionSegment
...
- (instancetype)initWithDictionaryRepresentation:(NSDictionary *)dictionary directory:(NSString *)directory {
    ...
    if (filename != nil) {
        NSURL *url = [GJSCRecordSessionSegment segmentURLForFilename:filename andDirectory:directory];
...
+ (GJSCRecordSessionSegment *)segmentWithURL:(NSURL *)url info:(NSDictionary *)info {
    return [[GJSCRecordSessionSegment alloc] initWithURL:url info:info];
}

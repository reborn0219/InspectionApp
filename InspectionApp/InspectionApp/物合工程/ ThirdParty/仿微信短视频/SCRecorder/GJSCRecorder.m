...
@interface GJSCRecorder() {
    ...
    GJSCSampleBufferHolder *_lastVideoBuffer;
    CIContext *_context;
...
- (id)init {
    ...
    if (self) {
        _sessionQueue = dispatch_queue_create("me.corsin.GJSCRecorder.RecordSession", nil);
        ...
        _lastVideoBuffer = [GJSCSampleBufferHolder new];
        _maxRecordDuration = kCMTimeInvalid;
        _resetZoomOnChangeDevice = YES;
        
        self.device = AVCaptureDevicePositionBack;
        _videoConfiguration = [GJSCVideoConfiguration new];
        _audioConfiguration = [GJSCAudioConfiguration new];
        _photoConfiguration = [GJSCPhotoConfiguration new];
        ...
        _context = [GJSCContext new].CIContext;
    }
...
+ (GJSCRecorder*)recorder {
    return [[GJSCRecorder alloc] init];
}
    ...
    if (session != nil) {
        ...
        if (![session.sessionPreset isEqualToString:_captureSessionPreset]) {
            if ([session canSetSessionPreset:_captureSessionPreset]) {
                ...
                newError = [GJSCRecorder createError:@"Cannot set session preset"];
            }
        }
        ...
        if (self.fastRecordMethodEnabled) {
            ...
            if (![session.outputs containsObject:_movieOutput]) {
                if ([session canAddOutput:_movieOutput]) {
                    ...
                    if (newError == nil) {
                        newError = [GJSCRecorder createError:@"Cannot add movieOutput inside the session"];
                    }
                }
            }
            ...
            if (self.videoConfiguration.enabled) {
                ...
                if (![session.outputs containsObject:_videoOutput]) {
                    if ([session canAddOutput:_videoOutput]) {
                        ...
                        if (newError == nil) {
                            newError = [GJSCRecorder createError:@"Cannot add videoOutput inside the session"];
                        }
                    }
                } else {
                    _videoOutputAdded = YES;
            ...
            if (self.audioConfiguration.enabled) {
                ...
                if (![session.outputs containsObject:_audioOutput]) {
                    if ([session canAddOutput:_audioOutput]) {
                        ...
                        if (newError == nil) {
                            newError = [GJSCRecorder createError:@"Cannot add audioOutput inside the sesssion"];
                        }
                    }
                } else {
                    _audioOutputAdded = YES;
        ...
        if (self.photoConfiguration.enabled) {
            ...
            if (![session.outputs containsObject:_photoOutput]) {
                if ([session canAddOutput:_photoOutput]) {
                    ...
                    if (newError == nil) {
                        newError = [GJSCRecorder createError:@"Cannot add photoOutput inside the session"];
                    }
                }
            }
        }
...
- (UIImage *)_imageFromSampleBufferHolder:(GJSCSampleBufferHolder *)sampleBufferHolder {
...
- (void)capturePhoto:(void(^)(NSError*, UIImage*))completionHandler {
    ...
    if (connection != nil) {
         ...
         ^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
             ...
             if (imageDataSampleBuffer != nil && error == nil) {
                 ...
                 if (jpegData) {
                     ...
                     if (completionHandler != nil) {
                         completionHandler([GJSCRecorder createError:@"Failed to create jpeg data"], nil);
                     }
                 }
             } else {
                 if (completionHandler != nil) {
        ...
        if (completionHandler != nil) {
            completionHandler([GJSCRecorder createError:@"Camera session not started or Photo disabled"], nil);
        }
    }
}
...
- (void)_progressTimerFired:(NSTimer *)progressTimer {
    ...
    if (CMTIME_COMPARE_INLINE(recordedDuration, !=, _lastMovieFileOutputTime)) {
        GJSCRecordSession *recordSession = _session;
        id<SCRecorderDelegate> delegate = self.delegate;
...
- (void)record {
    ...
    if ([GJSCRecorder isSessionQueue]) {
        block();
    } else {
        ...
        if (recordSession != nil) {
            if (recordSession.recordSegmentReady) {
                ...
                if (recordSession.isUsingMovieFileOutput) {
                    ...
                    [recordSession endSegmentWithInfo:info completionHandler:^(GJSCRecordSessionSegment *segment, NSError *error) {
                        id<SCRecorderDelegate> delegate = self.delegate;
...
+ (NSError*)createError:(NSString*)errorDescription {
    return [NSError errorWithDomain:@"GJSCRecorder" code:200 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
}

- (void)beginRecordSegmentIfNeeded:(GJSCRecordSession *)recordSession {
...
- (void)checkRecordSessionDuration:(GJSCRecordSession *)recordSession {
    ...
    if (CMTIME_IS_VALID(suggestedMaxRecordDuration)) {
        if (CMTIME_COMPARE_INLINE(currentRecordDuration, >=, suggestedMaxRecordDuration)) {
            ...
            dispatch_async(_sessionQueue, ^{
                [recordSession endSegmentWithInfo:[self _createSegmentInfo] completionHandler:^(GJSCRecordSessionSegment *segment, NSError *error) {
                    id<SCRecorderDelegate> delegate = self.delegate;
                    if ([delegate respondsToSelector:@selector(recorder:didCompleteSegment:inSession:error:)]) {
                        [delegate recorder:self didCompleteSegment:segment inSession:recordSession error:error];
                    }
...
- (GJSCFilter *)_transformFilterUsingBufferWidth:(size_t)bufferWidth bufferHeight:(size_t)bufferHeight mirrored:(BOOL)mirrored {
    if (_transformFilter == nil || _transformFilterBufferWidth != bufferWidth || _transformFilterBufferHeight != bufferHeight) {
        ...
        if (!shouldMirrorBuffer) {
            ...
            _transformFilter = [GJSCFilter filterWithAffineTransform:CGAffineTransformTranslate(CGAffineTransformScale(tx, -1, 1), -(CGFloat)bufferWidth, 0)];
        }
...
- (void)appendVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer toRecordSession:(GJSCRecordSession *)recordSession duration:(CMTime)duration connection:(AVCaptureConnection *)connection completion:(void(^)(BOOL success))completion {
    ...
    GJSCFilter *filterGroup = _videoConfiguration.filter;
    GJSCFilter *transformFilter = [self _transformFilterUsingBufferWidth:bufferWidth bufferHeight:bufferHeight mirrored:
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error {
    ...
    dispatch_async(_sessionQueue, ^{
        ...
        [_session appendRecordSegmentUrl:outputFileURL info:[self _createSegmentInfo] error:actualError completionHandler:^(GJSCRecordSessionSegment *segment, NSError *error) {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    if (captureOutput == _videoOutput) {
        ...
        GJSCImageView *imageRenderer = _GJSCImageView;
...
#pragma clang diagnostic pop
                    ...
                    *error = [GJSCRecorder createError:@"Failed to add input to capture session"];
                }
            }
        }
    }
...
- (AVCaptureDevice*)videoDevice {
    ...
    return [GJSCRecorderTools videoDeviceForPosition:_device];
}
...
- (void)setFlashMode:(SCFlashMode)flashMode {
    ...
    if (currentDevice.hasFlash) {
        ...
        error = [GJSCRecorder createError:@"Current device does not support flash"];
    }
...
- (void)setSession:(GJSCRecordSession *)recordSession {
...
- (void)setFrameRate:(CMTimeScale)framePerSeconds {
    ...
    if (device != nil) {
        NSError * error = nil;
        BOOL formatSupported = [GJSCRecorderTools formatInRange:device.activeFormat frameRate:framePerSeconds];
...
- (BOOL)setActiveFormatWithFrameRate:(CMTimeScale)frameRate width:(int)width andHeight:(int)height error:(NSError *__autoreleasing *)error {
    ...
    if (device != nil) {
        ...
        for (AVCaptureDeviceFormat *format in device.formats) {
            if ([GJSCRecorderTools formatInRange:format frameRate:frameRate dimensions:dimensions]) {
                if (bestFormat == nil) {
                    bestFormat = format;
                } else {
                    ...
                    if (currentDimensions.width < bestDimensions.width && currentDimensions.height < bestDimensions.height) {
                        ...
                        if ([GJSCRecorderTools maxFrameRateForFormat:bestFormat minFrameRate:frameRate] > [GJSCRecorderTools maxFrameRateForFormat:format minFrameRate:frameRate]) {
                            bestFormat = format;
                        }
                    }
                }
        ...
        if (bestFormat != nil) {
            ...
            if (error != nil) {
                *error = [GJSCRecorder createError:[NSString stringWithFormat:@"No format that supports framerate %d and dimensions %d/%d was found", (int)frameRate, dimensions.width, dimensions.height]];
            }
        }
    } else {
        if (error != nil) {
            *error = [GJSCRecorder createError:@"The camera must be initialized before setting active format"];
        }
    }
...
{
    return [GJSCRecorderTools videoDeviceForPosition:devicePosition].activeFormat.videoMaxZoomFactor;
}
...
+ (GJSCRecorder *)sharedRecorder {
    static GJSCRecorder *_sharedRecorder = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedRecorder = [GJSCRecorder new];
    });

...
#import "GJSCVideoPlayerView.h"

////////////////////////////////////////////////////////////
// PRIVATE DEFINITION
/////////////////////

@interface GJSCVideoPlayerView() {
    BOOL _holdPlayer;
...
@implementation GJSCVideoPlayerView
...
- (id)initWithPlayer:(GJSCPlayer *)thePlayer {
    self = [super init];
...
- (void)_commonInit:(GJSCPlayer *)player {
    _playerLayer = [AVPlayerLayer new];
    [self.layer insertSublayer:_playerLayer atIndex:0];

    BOOL holdPlayer = NO;
    if (player == nil && [GJSCVideoPlayerView autoCreatePlayerWhenNeeded]) {
        player = [GJSCPlayer player];
        holdPlayer = YES;
    }
    self.player = player;
...
- (void)setPlayer:(GJSCPlayer *)player {
    if (player != _player) {
        _player = player;

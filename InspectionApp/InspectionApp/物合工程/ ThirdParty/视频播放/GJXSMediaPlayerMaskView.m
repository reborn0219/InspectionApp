...
@interface GJXSMediaPlayerMaskView ()
...
@implementation GJXSMediaPlayerMaskView
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.startBtn setImage:[UIImage imageNamed:@"icon_n_09@2x"] forState:UIControlStateNormal];
        [self.startBtn setImage:[UIImage imageNamed:@"icon_n_09@2x1"] forState:UIControlStateSelected];
        ...
        [self.fullScreenBtn addTarget:self action:@selector(fullScreenBtnDidClicked) forControlEvents:UIControlEventTouchUpInside];
...
{
    if (self.fullscreenDelegate && [self.fullscreenDelegate respondsToSelector:@selector(fullScreenBtnDidClicked)]) {
        [self.fullscreenDelegate fullScreenBtnDidClicked];
    }
    else
    {
        NSLog(@"协议方案未实现");

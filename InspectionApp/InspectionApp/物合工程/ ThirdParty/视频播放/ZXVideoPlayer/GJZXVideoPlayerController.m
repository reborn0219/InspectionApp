...
@interface GJZXVideoPlayerController () <UIGestureRecognizerDelegate>

/// 播放器视图
@property (nonatomic, strong) GJZXVideoPlayerControlView *videoControl;
/// 是否已经全屏模式
...
@implementation GJZXVideoPlayerController
...
- (void)changeToFullScreenForOrientation:(UIDeviceOrientation)orientation
{
    ...
    self.frame = CGRectMake(0,HEIGHT/2 - kZXVideoPlayerOriginalHeight/2, kZXVideoPlayerOriginalWidth, kZXVideoPlayerOriginalHeight);
...
- (GJZXVideoPlayerControlView *)videoControl
{
    if (!_videoControl) {
        _videoControl = [[GJZXVideoPlayerControlView alloc] init];
    }
...
- (void)setFrame:(CGRect)frame
{
    ...
    [self.videoControl setFrame:CGRectMake(0, 0,SCREEN_WIDTH,SCREEN_HEIGHT)];

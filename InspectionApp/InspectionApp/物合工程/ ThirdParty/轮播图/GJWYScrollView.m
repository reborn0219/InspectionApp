...
@interface GJWYScrollView ()<UIScrollViewDelegate>
...
@implementation GJWYScrollView

#pragma mark- 初始化方法
//创建用来缓存图片的文件夹
+ (void)initialize {
    
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"GJWYScrollView"];
    BOOL isDir = NO;
...
+ (instancetype)scrollViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray {
    ...
    GJWYScrollView *wyScrollView = [[self alloc] init];
...
//            [_images addObject:[UIImage imageNamed:@"Placeholder"]];
    ...
    [self.currImageView sd_setImageWithURL:[NSURL URLWithString:_images[_currIndex]]];
...
- (void)downloadImages:(int)index {
    ...
    //从沙盒中取图片
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"GJWYScrollView"] stringByAppendingPathComponent:[key lastPathComponent]];

...
- (void)addBadgePoint:(NSInteger)pointRadius withPointPosition:(CGPoint)point {
    ...
    badgeView.tag = kTagBadgePointView;
    badgeView.layer.cornerRadius = pointRadius;
    badgeView.backgroundColor = HexRGB(0xf75388);
...
+ (UIView *)lineViewWithPointYY:(CGFloat)pointY{
    return [self lineViewWithPointYY:pointY andColor:HexRGB(0xc8c7cc)];
}
...
- (void)addLineUp:(BOOL)hasUp andDown:(BOOL)hasDown{
    [self addLineUp:hasUp andDown:hasDown andColor:HexRGB(0xc8c7cc)];
}
...
- (void)beginLoading{
    ...
    [self.loadingView mas_makeConstraints:^(GJMASConstraintMaker *make) {
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [_loopView mas_makeConstraints:^(GJMASConstraintMaker *make) {
            make.center.equalTo(self);
        }];
        [_monkeyView mas_makeConstraints:^(GJMASConstraintMaker *make) {
            make.center.equalTo(self);
        }];
...
- (void)configWithType:(EaseBlankPageType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block{
    ...
    [_tipLabel mas_makeConstraints:^(GJMASConstraintMaker *make) {
    ...
    if (hasError) {
        ...
        if (!_reloadButton) {
            ...
            [_reloadButton mas_makeConstraints:^(GJMASConstraintMaker *make) {
                make.centerX.equalTo(self);
        ...
        if ((blankPageType>=EaseBlankPageTypeProject_ALL)&&(blankPageType<=EaseBlankPageTypeProject_STARED)) {
            [_monkeyView mas_updateConstraints:^(GJMASConstraintMaker *make) {
                make.centerX.equalTo(self);
            ...
            UIButton *actionBtn=({
                ...
                button.backgroundColor=HexRGB(0x3BBD79);
            ...
            [actionBtn mas_makeConstraints:^(GJMASConstraintMaker *make) {

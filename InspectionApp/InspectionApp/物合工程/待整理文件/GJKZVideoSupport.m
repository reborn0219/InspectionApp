...
- (instancetype)initWithFrame:(CGRect)frame style:(KZVideoViewShowType)style; {
    if (self = [super initWithFrame:frame]) {
        _style = style;
        [GJKZVideoConfig motionBlurView:self];
        [self setupSubLayers];
    }
    return self;
}
...
- (void)setupRoundButton {
    self.backgroundColor = [UIColor clearColor];
    CGFloat bigPadding = 15/2.0;
    CGFloat smallPadding = 10;
    CGFloat allX = self.bounds.size.width;
    CGFloat allY = self.bounds.size.height;
    
    /*动画layer*/
    _animationLayer = [CAShapeLayer layer];
    _animationLayer.hidden = YES;
    CGFloat lineWidth = 6.0f;
    _animationLayer.path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2) radius:(allX/2.0-bigPadding)*2/2.0*1.6 - lineWidth+1.5 startAngle:M_PI*3/2 endAngle:M_PI*3/2+ M_PI*2 clockwise:YES].CGPath;
    _animationLayer.strokeColor = [UIColor greenColor].CGColor;
    ...
    /*大圆图片*/
    _bigCircleLayer = [CALayer layer];
    _bigCircleLayer.frame = CGRectMake(0, 0, (allX/2-bigPadding)*2, (allY/2-bigPadding)*2);
    _bigCircleLayer.position = self.center;
    _bigCircleLayer.backgroundColor = RGB(174, 169, 167).CGColor;
    _bigCircleLayer.cornerRadius = (allX/2-bigPadding);
    [self.layer addSublayer:_bigCircleLayer];
    
    /*小圆图片*/
    _smallCircleLayer = [CALayer layer];
    _smallCircleLayer.frame = CGRectMake(0, 0, (allX/2-bigPadding-smallPadding)*2, (allY/2-bigPadding-smallPadding)*2);
    _smallCircleLayer.position = self.center;
    _smallCircleLayer.backgroundColor = RGB(255, 255, 255).CGColor;
    _smallCircleLayer.cornerRadius = (allX/2-bigPadding-smallPadding);
    [self.layer addSublayer:_smallCircleLayer];
    
    [self.layer addSublayer:_animationLayer];
}

- (void)startAnimation{
//    POPBasicAnimation *basicAnimation = [POPBasicAnimation animationWithPropertyNamed:kPOPLayerScaleXY];
...
//    basicAnimation.toValue = [NSValue valueWithCGPoint:CGPointMake(1.6,1.6)];
//    [_bigCircleLayer pop_addAnimation:basicAnimation forKey:@"layerScaleXY"];
...
//    strokeAnimation.duration = kzRecordTime-1;
//    [_animationLayer pop_addAnimation:strokeAnimation forKey:@"layerStrokeAnimation"];
}
//
- (void)stopAnimation{

    _animationLayer.hidden = YES;
//    [_animationLayer pop_removeAnimationForKey:@"layerStrokeAnimation"];
//
//    POPBasicAnimation *basicAnimation = [POPBasicAnimation animationWithPropertyNamed:kPOPLayerScaleXY];
//    basicAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
...
- (void)setupSubViewsWithStyle:(KZVideoViewShowType)style {
   ...
   //[GJKZVideoConfig motionBlurView:self];
    ...
    _startBtn = [[KZRecordBtn alloc] initWithFrame:CGRectMake(0, 0, 85, 85) style:style];

...
@interface GJKZVideoViewController()<KZControllerBarDelegate,AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate> {
...
static GJKZVideoViewController *__currentVideoVC = nil;

@implementation GJKZVideoViewController
...
- (void)startAnimationWithType:(KZVideoViewShowType)showType {
    ...
    [keyWindow addSubview:self.view];
        ...
        self.view.hidden = NO;
        self.actionView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 0, CGRectGetHeight([GJKZVideoConfig viewFrameWithType:showType]));
...
- (void)endAniamtion {
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.actionView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 0, CGRectGetHeight([GJKZVideoConfig viewFrameWithType:_showType]));
    } completion:^(BOOL finished) {
        [self closeView];
    }];
}
...
- (void)setupSubViews {
    ...
    _actionView = [[UIView alloc] initWithFrame:[GJKZVideoConfig viewFrameWithType:_showType]];
    ...
    _videoView = [[UIView alloc] initWithFrame:CGRectMake(0,0,KScreenWigth,KScreenHeight)];
    [self.actionView addSubview:_videoView];
    
    _ctrlBar = [[KZControllerBar alloc] initWithFrame:CGRectMake(KScreenWigth/2-130/2, KScreenHeight - 150, 130, 130)];
    [_ctrlBar setupSubViewsWithStyle:_showType];
    _ctrlBar.delegate = self;
    _ctrlBar.backgroundColor = [UIColor clearColor];
    [self.actionView addSubview:_ctrlBar];

    _closeButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [_closeButton setImage:[UIImage imageNamed:@"repair_success"] forState:UIControlStateNormal];
    [_closeButton addTarget:self action:@selector(endAniamtion) forControlEvents:UIControlEventTouchUpInside];
    [self.actionView addSubview:_closeButton];
    [_closeButton mas_makeConstraints:^(GJMASConstraintMaker *make) {
        make.centerY.equalTo(_ctrlBar.mas_centerY);
        make.right.equalTo(_ctrlBar.mas_left).offset(-15);
    }];
...
- (void)setupVideo {
    ...
    _videoPreLayer.frame = CGRectMake(0, 0, KScreenWigth, KScreenHeight);
    _videoPreLayer.position = CGPointMake(KScreenWigth/2, (KScreenHeight)/2);
...
- (void)viewDidAppear {
    ...
    zoomLab.center = CGPointMake(_videoView.center.x, CGRectGetMaxY(_videoView.frame) - 200);
...
- (void)moveTopBarAction:(UIPanGestureRecognizer *)gesture {
    ...
    CGRect dafultFrame = [GJKZVideoConfig viewFrameWithType:_showType];
...
- (void)ctrollVideoDidCancel:(KZControllerBar *)controllerBar reason:(KZRecordCancelReason)reason{
    ...
    if (reason == KZRecordCancelReasonTimeShort) {
        [GJKZVideoConfig showHinInfo:@"录制时间过短" inView:_videoView frame:CGRectMake(0,CGRectGetHeight(_videoView.frame)/3*2,CGRectGetWidth(_videoView.frame),20) timeLong:1.0];
    }
//    NSLog(@"当前视频录制取消");
}
...
- (void)createWriter:(NSURL *)assetUrl {
    ...
    int videoWidth = [GJKZVideoConfig defualtVideoSize].width;
    int videoHeight = [GJKZVideoConfig defualtVideoSize].height;

...
@interface GJQueRenViewController ()
{
...
@implementation GJQueRenViewController
...
-(void)imgWZ
{
    ...
    [mgr POST:URL_LOCAL parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([[userDefaults objectForKey:@"is_autograph"] isEqualToString:@"F"]) {
            [self chuliGD];
        }else{
            [self WGGD];
        }
...
-(void)WGGD
{
    ...
    //获取头像数据
    //传mfa返回access_token
    NSString *accesstoken = [NSString stringWithModule:@"mlgj_api" Filename:@"repair" Action:@"edit_finished"];
    //1,请求管理者
    GJAFHTTPRequestOperationManager *mgr = [GJAFHTTPRequestOperationManager manager];
    mgr.requestSerializer = [AFHTTPRequestSerializer serializer];
    mgr.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"application/json", @"text/json", nil];
    ...
    params[@"rep[autograph]"]=[[NSUserDefaults standardUserDefaults] objectForKey:@"QMWJ"];

    
           GJLCLNetWork *network = [[GJLCLNetWork alloc]init];
    
            [network submitRequestNetWithInterface:@"" andM:@"mlgj_api" andF:@"repair" andA:@"edit_finished" andBodyOfRequestForKeyArr:[params allKeys] andValueArr:[params allValues] andBlock:^(id dictionary)
             {
...
-(void)chuliGD
{
    ...
    //获取头像数据
    //传mfa返回access_token
    NSString *accesstoken = [NSString stringWithModule:@"mlgj_api" Filename:@"repair" Action:@"receive"];
    //1,请求管理者
    GJAFHTTPRequestOperationManager *mgr = [GJAFHTTPRequestOperationManager manager];
    mgr.requestSerializer = [AFHTTPRequestSerializer serializer];
    mgr.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"application/json", @"text/json", nil];
    ...
    [network submitRequestNetWithInterface:@"" andM:@"mlgj_api" andF:@"repair" andA:@"receive" andBodyOfRequestForKeyArr:[params allKeys] andValueArr:[params allValues] andBlock:^(id dictionary)
     {
         NSLog(@"%@",[params allKeys]);
         NSLog(@"%@",[params allValues]);
         NSLog(@"%@",dictionary);

...
@interface GJYTKKeyValueStore()

@property (strong, nonatomic) GJFMDatabaseQueue * dbQueue;

@end

@implementation GJYTKKeyValueStore
...
- (id)initDBWithName:(NSString *)dbName {
    ...
    if (self) {
        ...
        _dbQueue = [GJFMDatabaseQueue databaseQueueWithPath:dbPath];
    }
    return self;
}
...
- (void)createTableWithName:(NSString *)tableName {
    if ([GJYTKKeyValueStore checkTableName:tableName] == NO) {
        return;
    }
    NSString * sql = [NSString stringWithFormat:CREATE_TABLE_SQL, tableName];
    __block BOOL result;
    [_dbQueue inDatabase:^(GJFMDatabase *db) {
...
- (YTKKeyValueItem *)getYTKKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    [_dbQueue inDatabase:^(GJFMDatabase *db) {
        GJFMResultSet * rs = [db executeQuery:sql, objectId];
        if ([rs next]) {
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    [_dbQueue inDatabase:^(GJFMDatabase *db) {
        GJFMResultSet * rs = [db executeQuery:sql];
        while ([rs next]) {
            YTKKeyValueItem * item = [[YTKKeyValueItem alloc] init];

...
@interface GJSliderViewController ()<UIGestureRecognizerDelegate>{
...
@implementation GJSliderViewController
...
+ (GJSliderViewController*)sharedSliderController
{
    static GJSliderViewController *sharedSVC;
...
- (void)initSubviews
{
    ...
    {
        _rightSideView.frame=CGRectMake(0,-40 ,self.view.frame.size.width,self.view.frame.size.height);
                _mainContentView.frame=CGRectMake(0,-40 ,self.view.frame.size.width,self.view.frame.size.height);
                _leftSideView.frame=CGRectMake(0,-40 ,self.view.frame.size.width,self.view.frame.size.height);
    }
...
- (void)initChildControllers:(UIViewController*)leftVC rightVC:(UIViewController*)rightVC
{
    if (_canShowRight&&rightVC!=nil) {
        ...
        if([[[UIDevice currentDevice] systemVersion] floatValue] < 7 )
        {
            rightVC.view.frame=CGRectMake(0, -40, rightVC.view.frame.size.width, rightVC.view.frame.size.height);
        }
    }
    if (_canShowLeft&&leftVC!=nil) {
        ...
        if([[[UIDevice currentDevice] systemVersion] floatValue] < 7 )
        {
            leftVC.view.frame=CGRectMake(0, -40, leftVC.view.frame.size.width, leftVC.view.frame.size.height);
            leftVC.view.backgroundColor=[UIColor colorWithRed:39 green:72 blue:14 alpha:1];
...
- (void)showContentControllerWithModel:(UIViewController*)main
{
    [self closeSideBar];
    
    UIViewController *controller = main;

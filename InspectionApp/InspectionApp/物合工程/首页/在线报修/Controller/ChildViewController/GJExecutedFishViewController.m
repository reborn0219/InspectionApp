//
//  GJExecutedFishViewController.m
//  物联宝管家
//
...
@interface GJExecutedFishViewController ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate,SYDatePickerDelegate>
{
    BOOL CellISOpen;
    BOOL ISMoney;
...
@implementation GJExecutedFishViewController
...
-(void)SubmitDid
{
    ...
    qm.WGDic=@{
               @"rep[repair_id]" : self.repairdIDStr,
               @"rep[come_in_time]" : rightOneLable.text,
               @"rep[come_out_time]" : rightTwoLable.text,
               @"rep[finished_time]" : @"",
               @"rep[service_charge]" : moneya,
               @"rep[material_fee]" : moneyb,
               @"rep[owner_cost]" : moneyc,
               @"rep[work_remarks]" : opinionView.text,
               @"rep[paid_type]" : weichatmoneyPay,
               };
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section == 0) {
        ...
        leftTwoLable.text = @"出户时间";
        if (_isAnBao) {
            leftOneLable.text = @"服务结束时间";
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < 3) {
        ...
        if (indexPath.row == 0) {
            ExecutedFishTableViewCell.rightLable.text = MaterialMoney;
...
-(void)SubmitDidClicked
{
    if (rightOneLable.text.length == 0) {
        [GJSVProgressHUD showErrorWithStatus:@"请选择入户时间！"];
    }else if(rightTwoLable.text.length == 0)
    {
        [GJSVProgressHUD showErrorWithStatus:@"请选择出户时间！"];
    }else if (MaterialMoney == nil)
    {
        [GJSVProgressHUD showErrorWithStatus:@"请输入材料费用！"];
    }else if (personMoney == nil)
    {
        [GJSVProgressHUD showErrorWithStatus:@"请输入人工费用！"];
    }
...
-(void)yesButtonDidClicked:(UIButton *)sender
{
    if (sender.tag == 1002) {
        ...
        }else
        {
            ...
            AllMoneyStr = [NSString stringWithFormat:@"￥%@",moneyText.text];
    ...
    }else
    {
        if (sender.tag == 1000) {
            ...
            {
                moneya = moneyText.text;
                MaterialMoney = [NSString stringWithFormat:@"￥%@",moneyText.text] ;
                floatMaterialMoney = [moneyText.text floatValue];

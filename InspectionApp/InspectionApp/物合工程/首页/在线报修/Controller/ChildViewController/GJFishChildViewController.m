...
@interface GJFishChildViewController ()<UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate>
{
    BOOL ISOpenOne;
    BOOL ISOpenTwo;
...
@implementation GJFishChildViewController
...
- (void)viewDidLoad {
    ...
    ISOpenFouth = YES;
    ISTIMERSTR = NO;
    self.tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, KScreenWigth, KScreenHeight)];
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    if ([_receiveDataDic[@"is_op"] isEqualToString:@"P"]) {
        if (section == 0)
        {
            if ([autograph isEqualToString:@"A"]) {
                ...
                if (Tuimage.size.width>WIDTH-40) {
                    ...
                    QMHeight=Tuimage.size.height/7*5+20+20;
                }
                return 120+QMHeight;
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if ([_receiveDataDic[@"is_op"] isEqualToString:@"P"]) {
        if(section == 0)
        {
            ...
            if ([autograph isEqualToString:@"A"]) {
                    UIView *QMview=[[UIView alloc]initWithFrame:CGRectMake(0,70+20 , WIDTH, WIDTH-40)];
                ...
                [QMview addSubview:kf];
                    ...
                    if (Tuimage.size.width>WIDTH-40) {
                        QMview.frame=CGRectMake(0, 90, WIDTH, (WIDTH-40)/(Tuimage.size.width/Tuimage.size.height )+20+20);
                        NameImg.frame=CGRectMake(0,30, WIDTH-40,(WIDTH-40)/(Tuimage.size.width/Tuimage.size.height ) );
                        QMHeight=(WIDTH-40)/(Tuimage.size.width/Tuimage.size.height )+20+20;
                        
                    }else{
                        QMview.frame=CGRectMake(0, 90, WIDTH, Tuimage.size.height/7*5+20+20);
                        
                        NameImg.frame=CGRectMake(0,30, Tuimage.size.width/7*5,Tuimage.size.height/7*5);
                        QMHeight=Tuimage.size.height/7*5+20+20;
                    }
...
//                self.tableview .estimatedRowHeight = QMHeight;
...
#pragma mark - 这里要添加名字和电话
    ...
    }else if(indexPath.section == i+1)
    {
        ...
        nameArray = @[@"维修师傅",@"联系电话",@"手续费",@"材料费",@"住户承担费用",@"入户时间",@"出户时间",@"备注"];
        if (_isAnBao) {
            nameArray = @[@"服务人",@"联系电话",@"手续费",@"材料费",@"住户承担费用",@"开始服务时间",@"结束服务时间",@"备注"];
        }
        NSArray *contentArray = [NSArray array];
        
        contentArray = @[_receiveDataDic[@"repaird"][@"repair_master_name"],_receiveDataDic[@"repaird"][@"repair_master_phone"],servicecharge,materialfee,ownercost,_receiveDataDic[@"repaird"][@"come_in_time"],_receiveDataDic[@"repaird"][@"come_out_time"],_receiveDataDic[@"repaird"][@"work_remarks"]];
...
-(void)playVideo
{
    ...
    if (self.FishChildDelegates && [self.FishChildDelegates respondsToSelector:@selector(PushVideoVCDidClicked:)]) {
        [self.FishChildDelegates PushVideoVCDidClicked:VideoUrl];
    }
    else
    {
        NSLog(@"协议方案未实现");

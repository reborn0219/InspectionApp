...
@interface GJExecutedViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate,UITextFieldDelegate,AVAudioPlayerDelegate>
{
    NSString *state;
...
@implementation GJExecutedViewController
{
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (videoArray.count != 0){
        ...
        fouthTableVCell.personnameLable.text = [NSString stringWithFormat:@"%@%@",@"维修人 : ",repairmasternameArray[indexPath.row]];
    ...
    else if(VoiceArray.count != 0)
    {
        ...
        secondacell.personnameLable.text = [NSString stringWithFormat:@"%@%@",@"维修人 : ",repairmasternameArray[indexPath.row]];
...
-(void)Notassigned
{
    ...
    community_id = model.community_id;
        ...
        [network requestNetWithInterface:@"" andM:@"mlgj_api" andF:@"repair" andA:@"repair_work_list" andBodyOfRequestForKeyArr:@[@"repair[property_id]",@"repair[community_id]",@"repair[repair_status]",@"start_num",@"per_pager_nums"] andValueArr:@[property_id,community_id,@"处理中",startNum,@"10"] andBlock:^(id dictionary)
         {
             NSLog(@"%@",dictionary);
             ...
             else if([state isEqualToString:@"1"])
             {
                 ...
                 exedataArray = dictionary[@"return_data"];
                 for (NSDictionary *dict in exedataArray) {
                     [descriptionArray addObject:dict[@"description"]];
                     [repairNumArray addObject:[NSString stringWithFormat:@"%@",dict[@"repair_no"]]];
                     [parentclassArray addObject:dict[@"parent_class"]];

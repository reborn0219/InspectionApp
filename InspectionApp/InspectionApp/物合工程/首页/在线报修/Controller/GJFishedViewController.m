...
@interface GJFishedViewController ()<UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate>
{
    NSString *state;
    ...
    UILabel *placeLable;
    NSArray *fishdataArray;
    NSMutableArray *fishAllDataArray;
...
@implementation GJFishedViewController
...
//    self.tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT - 153) style:UITableViewStylePlain];
    ...
    fishAllDataArray = [NSMutableArray array];
...
//        urgentimageView.image = [UIImage imageNamed:@"mlgj-2x50"];
    ...
    else if(VoiceArray.count != 0)
    {
        ...
        GJOverTableViewThreeCell *secondacell = [GJOverTableViewThreeCell createCellWithTableView:tableView withIdentifier:@"Secondflags"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"%@",fishAllDataArray[indexPath.row]);
    if (self.fishDelegates && [self.fishDelegates respondsToSelector:@selector(FishWageDidClicked:)]) {
        [self.fishDelegates FishWageDidClicked:fishAllDataArray[indexPath.row]];
    }
    else
    {
        NSLog(@"协议方案未实现");
...
-(void)Notassigned
{
    ...
    {
        if ([userDefaults objectForKey:@"AdminWagefishdataArray"] != nil) {
            [self removeData];
            [placeLable removeFromSuperview];
            [placeimageView removeFromSuperview];
            if ([[userDefaults objectForKey:@"role"]isEqualToString:@"admin"]) {
                fishdataArray = [userDefaults objectForKey:@"AdminWagefishdataArray"];
            }else
            {
                fishdataArray = [userDefaults objectForKey:@"NormalWagefishdataArray"];
            }
            for (NSDictionary *dict in fishdataArray) {
            ...
            {
                [fishAllDataArray addObject:fishdataArray[i]];
            }
    ...
    NSLog(@"%@",community_id);
        ...
        [network requestNetWithInterface:@"" andM:@"mlgj_api" andF:@"repair" andA:@"repair_work_list" andBodyOfRequestForKeyArr:@[@"repair[property_id]",@"repair[community_id]",@"repair[repair_status]",@"start_num",@"per_pager_nums"] andValueArr:@[property_id,community_id,@"已处理",startNum,@"10"] andBlock:^(id dictionary)
         {
             NSLog(@"%@",startNum);
             ...
             if ([state isEqualToString:@"5"] || [state isEqualToString:@"4"] || [state isEqualToString:@"2"]) {
                 ...
                 dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                     if (self.fishDelegates && [self.fishDelegates respondsToSelector:@selector(PushLoginVCDidClicked)]) {
                         [self.fishDelegates PushLoginVCDidClicked];
                     }
                     else
                     {
                         NSLog(@"协议方案未实现");
             ...
             else if([state isEqualToString:@"1"])
             {
                 ...
                 [placeLable removeFromSuperview];
                 [placeimageView removeFromSuperview];
                 fishdataArray = [NSArray array];
                 fishdataArray = dictionary[@"return_data"];
                 for (NSDictionary *dict in fishdataArray) {
                     [descriptionArray addObject:dict[@"description"]];
                     [repairNumArray addObject:[NSString stringWithFormat:@"%@",dict[@"repair_no"]]];
                     [parentclassArray addObject:dict[@"parent_class"]];
...
//                 if ([[userDefaults objectForKey:@"role"]isEqualToString:@"admin"]) {
//                     [userDefaults setObject:fishAllDataArray forKey:@"AdminWagefishdataArray"];
//                 }else
//                 {
//                     [userDefaults setObject:fishAllDataArray forKey:@"NormalWagefishdataArray"];
//                 }
...
-(void)removeData
{
    ...
    [fishAllDataArray removeAllObjects];
...
-(void)videoButtonDidClicked:(UIButton *)bt
{
    ...
    if (self.fishDelegates && [self.fishDelegates respondsToSelector:@selector(PushVideoVCDidClicked:)]) {
        [self.fishDelegates PushVideoVCDidClicked:VideoUrl];
    }
    else
    {
        NSLog(@"协议方案未实现");
...
-(void)imagebuttonDidClicked:(UIButton *)sender
{
    ...
    if (self.fishDelegates && [self.fishDelegates respondsToSelector:@selector(pushImagebrowserDidClicked:imageTag:)]) {
        [self.fishDelegates pushImagebrowserDidClicked:imagemutableArray imageTag:sender.tag - 2000];
    }
    else
    {
        NSLog(@"协议方案未实现");

...
@interface GJUnexecutedViewController ()<UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate,UIAlertViewDelegate,UITextViewDelegate,UITextFieldDelegate,workNameIDDelegates,CuiPickViewDelegate,worknameIDDelegates>
{
    NSString *state;
...
@implementation GJUnexecutedViewController
...
-(void)Notassigned
{
    ...
    if (community_id == nil) {
        ...
        [network requestNetWithInterface:@"" andM:@"mlgj_api" andF:@"repair" andA:@"repair_work_list" andBodyOfRequestForKeyArr:@[@"repair[property_id]",@"repair[community_id]",@"repair[repair_status]",@"start_num",@"per_pager_nums"] andValueArr:@[property_id,community_id,@"待处理",startNum,@"10"] andBlock:^(id dictionary)
         {
             NSLog(@"%@",dictionary);
...
///转移工单-ls
//提交工单
-(void)TransferButtoDidClicked_ls:(NSInteger) tag
{
    ...
    }else
    {
        ...
        [network submitRequestNetWithInterface:@"" andM:@"mlgj_api" andF:@"security" andA:@"repair_transfer" andBodyOfRequestForKeyArr:@[@"repair_id",@"repair_user_id",@"remarks",@"type"] andValueArr:@[repairID,theworkID,opinionView.text,@"E"] andBlock:^(id dictionary)

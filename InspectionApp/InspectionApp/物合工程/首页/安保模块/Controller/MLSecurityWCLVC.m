...
@interface MLSecurityWCLVC ()<UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate,UIAlertViewDelegate,UITextViewDelegate,UITextFieldDelegate,workNameIDDelegates>
{
    NSString *state;
...
@implementation MLSecurityWCLVC
-(void)viewWillAppear:(BOOL)animated
{
    WAGEISSUCCESS = YES;
    [self.tableview.mj_header beginRefreshing];
    
    NSLog(@"角色类型:%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"role"]);
...
//        NSDictionary * dic = @{@"m":@""};
        [network requestNetWithInterface:@"" andM:@"mlgj_api" andF:@"security" andA:@"security_list" andBodyOfRequestForKeyArr:@[@"property_id",@"community_id",@"repair_status",@"start_num",@"per_pager_nums"] andValueArr:@[property_id,community_id,@"未处理",startNum,@"10"] andBlock:^(id dictionary)
         {
             NSLog(@"%@",dictionary);
...
-(void)OnebuttonDdiClicked:(UIButton *)sender
{
    ...
    }else
    {
        ...
        GJLCLNetWork *network = [[GJLCLNetWork alloc]init];
        [network submitRequestNetWithInterface:@"" andM:@"mlgj_api" andF:@"security" andA:@"grab" andBodyOfRequestForKeyArr:@[@"repair_id",@"property_id"] andValueArr:@[repairID,property_ids] andBlock:^(id dictionary)
         {
...
-(void)YESButtonDidClicked:(UIButton *)sender
{
    ...
    else
    {
        ...
        [network submitRequestNetWithInterface:@"" andM:@"mlgj_api" andF:@"security" andA:@"fen_pei" andBodyOfRequestForKeyArr:@[@"repair_id",@"repair_user_id",@"remarks"] andValueArr:@[repairID,theworkID,opinionView.text] andBlock:^(id dictionary)
         {
...
-(void)getMaintenancemaster
{
    ...
    GJLCLNetWork *network = [[GJLCLNetWork alloc]init];
    [network requestNetWithInterface:@"" andM:@"mlgj_api" andF:@"security" andA:@"repair_who" andBodyOfRequestForKeyArr:@[@"property_id",@"community_id",@"start_num",@"per_pager_nums"] andValueArr:@[property_ids,community_ids,startNum,@"100"] andBlock:^(id dictionary)
     {
         state = [NSString stringWithFormat:@"%@",dictionary[@"state"]];
         if ( [state isEqualToString:@"4"] || [state isEqualToString:@"2"]) {
             [GJSVProgressHUD showErrorWithStatus:@"登录失效，请重新登录"];
...
-(void)SubmitMatterData
{
    ...
    NSString *accesstoken = [NSString stringWithModule:@"mlgj_api" Filename:@"sucurity" Action:@"do_repair"];
    //1,请求管理者
    GJAFHTTPRequestOperationManager *mgr = [GJAFHTTPRequestOperationManager manager];

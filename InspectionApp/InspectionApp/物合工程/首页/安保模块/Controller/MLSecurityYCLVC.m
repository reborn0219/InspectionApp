...
@interface MLSecurityYCLVC ()<UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate>
{
    NSString *state;
...
@implementation MLSecurityYCLVC
...
-(void)Notassigned
{
   ...
   [network requestNetWithInterface:@"" andM:@"mlgj_api" andF:@"security" andA:@"security_list" andBodyOfRequestForKeyArr:@[@"property_id",@"community_id",@"repair_status",@"start_num",@"per_pager_nums"] andValueArr:@[property_id,community_id,@"已处理",startNum,@"10"] andBlock:^(id dictionary)
     {
         NSLog(@"%@",startNum);
         ...
         else if([state isEqualToString:@"1"])
         {
             ...
             fishdataArray = dictionary[@"return_data"][@"repair_data"];
